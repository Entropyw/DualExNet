...
def select_model(args, device):
    ...
    if model_id == 0:
        ...
        from models.team02_DualExNet import DualExNet
        name, data_range = f"{model_id:02}_DualExNet", 1.0
        model_path = os.path.join('model_zoo', 'DualExNet.pth')
        tile = 512
        model = DualExNet(
            inp_channels=3,
            out_channels=3,
        ...
        # Backward-compatible checkpoint key remap after branch renaming.
        remapped_state_dict = {}
        for key, value in state_dict.items():
            new_key = key
            if new_key.startswith('swin_branch.'):
                new_key = 'local_branch.' + new_key[len('swin_branch.'):]
            if new_key.startswith('restormer_branch.'):
                new_key = 'global_branch.' + new_key[len('restormer_branch.'):]
